...
#init PID controllers for pitch/roll
pid_p = PID_Ctrl()
pid_r = PID_Ctrl()
pid_p.p = pid_r.p = 0.5
pid_p.i = pid_r.i = 0.05
pid_p.d = pid_r.d = 0.01
pid_p.int_en = pid_r.int_en = True
...
try:
   ...
   while True:
      ...
      if rc_data[0]:
		 ...
		 #sonar data analysis
		 #needs to be adjusted according sensor placement
		 fw_direction = sonar[0]
		 bw_direction = sonar[1]
		 lt_direction = sonar[3]
		 rt_direction = sonar[4]
		 
		 #all these is only for testing
		 
		 #check for critical region
		 safe_distance = 1.0
		 if (fw_direction > safe_distance):
			fw_direction = safe_distance
		 if (bw_direction > safe_distance):
			bw_direction = safe_distance
  		 if (lt_direction > safe_distance):
			lt_direction = safe_distance
		 if (rt_direction > safe_distance):
			rt_direction = safe_distance
         
		 #tolerance for a safe distance
		 tolerance = 0.2
		 
		 if abs(fw_direction - bw_direction) > tolerance:
			#PID control
			new_pitch_stick = pid_p.control(fw_direction/bw_direction, 1.0)
		 if abs(lt_direction - rt_direction) > tolerance:
			new_roll_stick = pid_p.control(lt_direction/rt_direction, 1.0)
			
		 if (abs(new_pitch_stick) < 0.5):
			pr_sticks[0] = new_pitch_stick
			if (pid_p.int_en == False): 
				pid_p.int_en = True
		 else:
			pr_sticks[0] = 0.5
			pid_p.int_en = False
		 ...
		 if (abs(new_roll_stick) < 0.5):
			pr_sticks[1] = new_roll_stick
			if (pid_r.int_en == False): 
				pid_r.int_en = True
		 else:
			pr_sticks[1] = 0.5 
			pid_r.int_en = False
